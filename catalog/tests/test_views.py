...
class LoanedBookInstancesByUserListViewTest(TestCase):
    def setUp(self):
        # Create two users
        user1 = User.objects.create_user(username='user1', password='1X<ISRUkw+tuK')
        user2 = User.objects.create_user(username='user2', password='2HJ1vRV0Z&3iD')
        ...
        book.genre.set(genre_objects_for_book) # Direct assignment of many-to-many types not allowed.
        ...
        for book_copy in range(number_of_book_copies):
            ...
            borrower = user1 if book_copy % 2 else user2
            status = 'm'
...
class RenewBookInstancesViewTest(TestCase):
    def setUp(self):
        ...
        return_date = timezone.localtime() + datetime.timedelta(days=5)
    ...
    def test_404_for_invalid_book_if_logged_in(self):
        ...
        uid = uuid.uuid4()
        login = self.client.login(username='user2', password='2HJ1vRV0Z&3iD')
        response = self.client.get(reverse('renew-book-librarian', kwargs={'pk': uid}))
        # Check that it lets us login - we are a librarian, so we can view any users book
